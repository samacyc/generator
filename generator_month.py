...
def main (rush_hour , rush_range , revenue , percentag_e ,die_hour, die_range , ordervalue1 , ordervalue2,per2,tocart,checkout,purchased ,link , link1 , link2 ) :
    ...
    for i in range(30) : 
        if (i != 30) : 
            revenue_per_day.append(random.uniform(((revenue*(1-0.3))/k) ,((revenue*(1+0.3))/k)))
    ...
    for i in range(30) : 
        ...
        for i in range (len(sale)) : 
            sales_week.append(sale[i])
            each_day.append(data[i])
        print('*******************************')
    X = np.arange(30)
    print(len(sales_week))
    print(len(revenue_per_day))
    with open('./config_month.json', 'w') as outfile:
